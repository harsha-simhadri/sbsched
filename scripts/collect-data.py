...
for sched_id in range(0, len(scheds)):
  for runs in range(0, int(args[1])):
    command = " hugectl --heap=2M numactl -i "+ sockets + " " + path + " " + scheds[sched_id] 
    for more_args in range(3, len(sys.argv)):
    ...
    for line in output:
      ...
      if line.startswith("L3_MISSES: "):
        words =  line.split(" ")
        l3_misses[sched_id].append(int(words[1]))
        print l3_misses
...
for sched_id in range(0, len(scheds)):
  fout.write("\n")
  for runs in range(0, int(args[1])):
    fout.write(str(l3_misses[sched_id][runs]))
    fout.write("   ")
fout.write("\n")
...
for sched_id in range(0, len(scheds)):
  ...
  l3_misses_filt[sched_id].remove(max(l3_misses_filt[sched_id]))
  l3_misses_filt[sched_id].remove(min(l3_misses_filt[sched_id]))
  sum=0.0
  average=0.0
  for n in l3_misses_filt[sched_id]:
    sum = sum + n
  average = sum/(1000000*len(l3_misses_filt[sched_id]))
