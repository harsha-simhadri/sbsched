...
int
new_counter (int counter, int cpu, int group)
{
  int fd = -1;
  struct perf_event_attr attr;

  memcpy (&attr, &(default_attrs[counter]), sizeof (struct perf_event_attr));
  attr.inherit = 1;
  attr.inherit_stat = 1;
  attr.disabled = 1;
  attr.enable_on_exec = 0;
  fd = sys_perf_event_open (&attr, gettid(), cpu, group, 0);

  if (fd<0)
    fprintf (stderr, "error code: %d proc: %d", errno, cpu);
  assert (fd>=0);
  return fd;
}

int
enable_counter (int fd)
{
  return ioctl(fd, PERF_EVENT_IOC_ENABLE);
}

int
disable_counter (int fd)
{
...
{
  uint64_t value=0;
  uint read_size=read(fd, &value, sizeof(uint64_t));
  assert (read_size == sizeof(uint64_t));
  return value;
}

uint64_t
read_and_reset_counter (int fd)
{
  uint64_t value=0;
  uint read_size = read(fd, &value, sizeof(uint64_t));
  assert (read_size == sizeof(uint64_t));
  return ioctl(fd, PERF_EVENT_IOC_RESET);  
