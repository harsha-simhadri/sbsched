...
#define SEQNUM 1024
#define REPEATS 5
#define SEQ_SIZE (1<<18)

class numProcTest : public SizedJob {
  char* A; int n;
  int stride; int offset; int depth; 
  int stage;

public:  
  numProcTest(char* A_, int n_, bool del=true)
    : SizedJob(del), A(A_), n(n_), stride(1), offset(0), depth(0), stage(0) {}
  
  numProcTest(char* A_, int n_, int stride_, int offset_, int depth_, int stage_, bool del=true) 
    : SizedJob(del), A(A_), n(n_), stride(stride_), offset(offset_), depth(depth_), stage(stage_) {}

  numProcTest(numProcTest* from, bool del=true)
  ...
  void function() {
	...
	binary_fork(new numProcTest(A,n,2*stride,offset,depth+1,stage),
		    new numProcTest(A,n,2*stride,offset+stride,depth+1,stage),
		    new numProcTest(this));
      } 
    }
  }
};
